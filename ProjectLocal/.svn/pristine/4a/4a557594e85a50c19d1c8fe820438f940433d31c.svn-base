...
public class SelectFormation2 extends GameState{
	...
	public void draw(Graphics2D g) {
		...
		if(this.gsm.getBuilder().getMatch().teamB instanceof UserDefinedTeam)
			...
			g.drawString("Select Formation for Computer's Team", 25, 200);
		}
		// draw menu options
		g.setFont(font);
	...
	public void keyPressed(int k) {
		...
		if(flag == 1){
			if(k == KeyEvent.VK_ENTER){
				gsm.getBuilder().Team2makeLines(gsm.getBuilder().getMatch().teamB, noOfDefenders, noOfMidFielders, noOfAttackers);
				gsm.setState(4);
			}
		}
